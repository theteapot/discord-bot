module.exports = function main() {
    ...
    */ 
    ...
    /*
     * Refreshing the tokens every five minutes
    */
    ...
    function playlistPicker(user) {
        ...
        function(data) {
            ...
            rl.question('Enter the index (e.g. 1) of the desired playlist: ', (answer) => {
                ...
                playlist = data.body.items[answer].id
                rl.close();
            })
        }, 
        function (err) {
    ...
    */ 
    ...
    function getNextPos(callback) {
        var playlistPromise = rp(`http://localhost:9999/playlist/${playlist}/${id}`).then(function (playlistResponse) {
            var playingPromise = rp('http://localhost:9999/playing').then(function (playing) {
                playlistObj = JSON.parse(playlistResponse);
                playing  = JSON.parse(playing);
                //Check to see where in the playlist the currently playing song is
                for (var index in playlistObj.body.tracks.items) {
                    if (playlistObj.body.tracks.items[index].track.name === playing.name) {
