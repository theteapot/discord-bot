...
app.get('/add/:trackUris', function (req, res) {
    var tracks = req.params;
    console.log('Got add request', tracks);
    res.send('Success');
});
...
function searchTrack(query, callback) {
    spotifyApi.searchTracks(`track:${query.track} artist:${query.artist}`, {limit:3})
    .then(function(data) {
...
function addTracks (trackArray) {
    spotifyApi.addTracksToPlaylist('whistlingteapot', '6ekP7XeoMqvAeOu4ZRxbIq', trackArray).then(function(data) {
